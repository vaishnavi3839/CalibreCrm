...
export function AppShell({
  ...
  const homePath = dashboardPathForRole(user.role.name);
  ...
  return (
    <div className="min-h-screen pb-[env(safe-area-inset-bottom)]">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-64 max-w-[85vw] flex-col bg-navy-900 text-white transition-transform duration-200 ease-out md:z-30 md:w-64 md:max-w-none md:translate-x-0 md:pointer-events-auto",
          // Off-screen drawers must not steal taps on Android WebView
          mobileNavOpen
      ...
      >
        <div className="flex items-center gap-3 border-b border-white/10 px-3 py-4 sm:px-4 sm:py-5">
          ...
          <button
            type="button"
            aria-label="Close menu"
            className="min-h-11 min-w-11 rounded-lg p-2 text-white/70 hover:bg-white/10 hover:text-white md:hidden"
            onClick={() => setMobileNavOpen(false)}
          >
            <X className="h-5 w-5" />
          </button>
        ...
        >
          <Link href="/app/profile" className="mb-3 flex min-h-11 items-center gap-3 rounded-xl px-1 py-1 hover:bg-white/5">
          ...
          <button
            ...
            className="flex min-h-11 w-full items-center gap-2 rounded-xl bg-white/5 px-3 py-2 text-sm text-white/80 hover:bg-white/10"
          >
      ...
      <div className="md:pl-64">
        <header
          className="sticky top-0 z-20 border-b border-navy-900/5 bg-white/95 backdrop-blur-md"
          style={{ paddingTop: "env(safe-area-inset-top)" }}
        >
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-3 py-3 sm:gap-3 sm:px-6">
            <div className="flex min-w-0 flex-1 items-center gap-1.5 sm:gap-2">
              <button
                ...
                className="flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded-xl border border-cloud-200 bg-white text-navy-900 shadow-sm md:hidden"
              ...
              {showBack && (
                <button
                  type="button"
                  aria-label="Go back"
                  className="flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded-xl border border-cloud-200 bg-white text-navy-900 shadow-sm"
                  onClick={goBack}
                >
              ...
              <div className="min-w-0">
                <h1 className="truncate text-base font-semibold text-navy-900 sm:text-xl">
                  {title || "Dashboard"}
                </h1>
                {subtitle && (
            ...
            <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
              ...
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={avatarSrc(user)} alt="" className="h-9 w-9 rounded-full border border-cloud-200 object-cover" />
              </Link>
              <Link
                href="/app/notifications"
                className="flex min-h-11 min-w-11 items-center justify-center rounded-full bg-cloud-100 text-navy-800"
